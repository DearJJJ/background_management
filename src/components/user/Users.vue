  ...
  <div>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark" content="修改角色" placement="top" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="mini" round @click="showEditDialog(scope.row.id)"></el-button>
            </el-tooltip>
            <!-- 删除角色 -->
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="assignRole(userInfo.id)">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  data() {
    var checkEmail = (rule, value, cb) => {
      const regEmail = /^\w+@[a-zA-Z0-9]+((\.[a-z0-9A-Z]{1,})+)$/;
  ...
  methods: {
    ...
    async stateChanged(dataRow) {
      const { data: res } = await this.$http.put(`users/${dataRow.id}/state/${dataRow.mg_state}`);
      if (res.meta.status !== 200) {
        dataRow.mg_state = !dataRow.mg_state;
        return this.$message('用户状态更新失败！');
      }
